...
n=2000
k = 4
alpha_list = c(0, 0.25, 0.5, 0.75, 1, 1.25)
num_iter_coef = 100
ones = rep(1,n)
M = matrix(1, n, n)
...
results = NULL
for (alpha_bar in alpha_list){
    for (seed in seed_list){
      if (alpha_bar == 0){
        file_seq = paste("_n_", n, "_k_", k, "_seed_", seed, ".RData", sep="")
      } else {
        file_seq = paste("_n_", n, "_k_", k, "_alpha_", alpha_bar,  "_seed_", seed, ".RData", sep="")
      }
      print(file_seq)
      # generate data
      ...
      if (file.exists(data_f)){
        ...
        df = generate_data(n, k, seed, alpha_bar, 0)
        save(df, file = data_f)
      }
      edge_den = sum(abs(df$A_true))/n^2
      ...
      results = rbind(results, c(e_theta, e_eta, e_Z, e_v, iter_Z, iter_v, runtime_z, runtime_v, n, k, seed, alpha_bar, edge_den))
    }
    save(results, file = paste("./res/synthetic/separate_estimation_", mode, "_n_", n, "_k_", k, "_alpha_", alpha_bar, ".RData", sep=""))
}
...
for (alpha_bar in alpha_list){
  for (seed in seed_list){
    ...
    results = rbind(results, c(e_theta, e_eta, e_Z, e_v, seed, alpha_bar, edge_den))
  }
  save(results, file = paste("./res/synthetic/one_step_joint_estimation_", mode, "_n_", n, "_k_", k, "_alpha_", alpha_bar, ".RData", sep=""))
}
...
for (alpha_bar in alpha_list){
  ...
  save(results, file = paste("./res/synthetic/joint_estimation_", mode, "_n_", n, "_k_", k, "_alpha_", alpha_bar, ".RData", sep=""))
}
...
load(paste("./res/synthetic/separate_estimation_", mode, "_n_", n, "_k_", k, "_alpha_", 1.25, ".RData", sep=""))
...
colnames(result) = c("Theta", "Eta", "Z", "v", "seed", "alpha", "Density_seed")
result_sep = result %>% group_by(alpha) %>% summarise(Density = mean(Density_seed), 
                                                   Z_mean = mean(Z), Z_sd = sd(Z), 
                                                   v_mean = mean(v), v_sd = sd(v), .groups = 'drop')
result_sep$Algorithm = rep("Separate Estimation", dim(result_sep)[1])

# load one-step-joint estimation 
load(paste("./res/synthetic/one_step_joint_estimation_", mode, "_n_", n, "_k_", k, "_alpha_", 1.25, ".RData", sep=""))
result = data.frame(results)
colnames(result) = c("Theta", "Eta", "Z", "v", "seed", "alpha", "Density_seed")
result_one_step = result %>% group_by(alpha) %>% summarise(Density = mean(Density_seed), 
                                                   Z_mean = mean(Z), Z_sd = sd(Z), 
                                                   v_mean = mean(v), v_sd = sd(v), .groups = 'drop')
result_one_step$Algorithm = rep("One-step-joint Estimation", dim(result_one_step)[1])

# load joint estimation 
load(paste("./res/synthetic/joint_estimation_", mode, "_n_", n, "_k_", k, "_alpha_", 1.25, ".RData", sep=""))
result = data.frame(results)
colnames(result) = c("Theta", "Eta", "Z", "v", "seed", "alpha", "Density_seed")
result_joint = result %>% group_by(alpha) %>% summarise(Density = mean(Density_seed), 
                                                           Z_mean = mean(Z), Z_sd = sd(Z), 
                                                           v_mean = mean(v), v_sd = sd(v), .groups = 'drop')
...
p_z <- ggplot(result, aes(x=Density, y=Z_mean, colour=Algorithm)) + 
    geom_errorbar(aes(ymin=Z_mean-Z_sd, ymax=Z_mean+Z_sd), width=.04, position=pd) +
    geom_line(aes(linetype = Algorithm), position=pd, size=0.5) +
    geom_point(aes(shape = Algorithm), size=1.5,position=pd) +
    xlab("Network Density") + ylab("Relative Error - Z")
    theme(text = element_text(size=11))
p_v <- ggplot(result, aes(x=Density, y=v_mean, colour=Algorithm)) + 
  geom_errorbar(aes(ymin=v_mean-v_sd, ymax=v_mean+v_sd), width=.04,position=pd) +
  geom_line(aes(linetype = Algorithm), position=pd, size=0.5) +
  geom_point(aes(shape = Algorithm), size=1.5,position=pd) +
  xlab("Network Density") + ylab("Relative Error - v")+
  theme(text = element_text(size=11))
pp[[1]] = p_z
pp[[2]] = p_v
p = ggarrange(plotlist=pp, ncol=2, nrow=1, common.legend = TRUE, legend="top")
p
pdf("./plots/network_density_comparison_n_2000_k_4.pdf", width=7, height = 3.8)
print(p)
dev.off()
